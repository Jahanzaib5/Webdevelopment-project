...
if(($_POST['name'])== '' || ($_POST['email'])=='' || empty($_Post['subject'])=='')
	...
	if (!mysqli_select_db($con, $db)){
	   // Create database
	   if (mysqli_query($con, "CREATE DATABASE university_DB")){
	   ...
	   $sql = "CREATE TABLE university(Name varchar(255), Email varchar(255), Subject varchar(255), Comment varchar(255))";
	   // Execute query
	   mysqli_query($con, $sql);
	}
	else{
	   mysqli_select_db($con, $db);
	   $sql="INSERT INTO university(Name, Email, Subject, Comment) VALUES('$name', '$email', '$subject', '$comment')";
